...
export default function MemberRoleManagementDialog({
  ...
  const [storageLimitMb, setStorageLimitMb] = useState<number>(DEFAULT_STORAGE_LIMIT_MB);
  ...
  useEffect(() => {
    if (open && member) {
      ...
      setStorageLimitMb(member.storage_limit_mb ?? DEFAULT_STORAGE_LIMIT_MB);
      setEditingLimits(false);
      Promise.all([fetchGroups(member.id), fetchStorageUsed(member.id)])
  ...
  const fetchStorageUsed = async (userId: string) => {
    // Sum file sizes from submission_history and project_resources
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      await supabase.from('activity_logs').insert({
        user_id: user!.id,
        user_name: currentProfile?.full_name || 'Admin',
  ...
  const handleSaveLimits = async () => {
    ...
    try {
      ...
      await supabase.from('activity_logs').insert({
        ...
        action_type: 'member',
        description: `Cập nhật giới hạn cho ${member.full_name}: ${projectLimit} projects, ${storageLimitMb}MB`,
        metadata: { target_user_id: member.id, project_limit: projectLimit, storage_limit_mb: storageLimitMb },
      });
            ...
            <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <StatCard label="Dung lượng đã dùng" value={`${storageUsedMb} MB`} icon={HardDrive} />
                      ...
                      {groups.map(g => (
                        <div key={g.id} className="flex items-center justify-between p-2.5 rounded-lg border bg-card hover:bg-muted/30 transition-colors">
                          <div className="flex items-center gap-2.5 min-w-0">
                            {g.image_url ? (
                              <img src={g.image_url} alt={g.name} className="w-8 h-8 rounded-md object-cover" />
                            ) : (
                      ...
                      {editingLimits ? (
                        <>
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1.5">
                              <Label className="text-xs">Dung lượng (MB)</Label>
                              <Input type="number" min={0} value={storageLimitMb}
                        ...
                        <>
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              <p className="text-xs text-muted-foreground">Dung lượng lưu trữ</p>
                              <p className="text-lg font-bold">{member.storage_limit_mb ?? DEFAULT_STORAGE_LIMIT_MB} MB</p>
                              <p className="text-xs text-muted-foreground">Đã dùng: {storageUsedMb} MB</p>
                            </div>
                          </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <br />• Dung lượng: {DEFAULT_STORAGE_LIMIT_MB} MB
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
