...
import { Github, Linkedin, Facebook, Globe, Mail, GraduationCap, BookOpen, Calendar, ArrowLeft, Loader2, FolderKanban } from 'lucide-react';
...
export default function PublicProfile() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Load public groups this user belongs to
      const { data: memberData } = await supabase
        .from('group_members')
        .select('group_id, groups!inner(id, name, slug, description, image_url, is_public)')
        .eq('user_id', data.id);

      if (memberData) {
        const publicGroups = memberData
          .filter((m: any) => m.groups?.is_public)
          .map((m: any) => m.groups as PublicGroup);
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 mb-8">
          <div className="shrink-0">
            <UserAvatar
              ...
              size="xl"
              className="w-28 h-28 border-4 border-background shadow-xl text-2xl"
            />
          </div>
