...
export default function PublicProfile() {
  ...
  const getImageUrl = (path: string) => supabase.storage.from('profile-achievements').getPublicUrl(path).data.publicUrl;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-muted-foreground animate-pulse font-medium">Đang tải trang cá nhân...</p>
        </div>
      </div>
    );
  }
                  ...
                  <div className={`flex flex-wrap items-center justify-center md:justify-start gap-2 mt-4 transition-all duration-500 delay-[600ms] ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    {profile.student_id && (
                      <div className="flex items-center gap-1.5 text-xs bg-muted/80 rounded-full px-3 py-1.5 text-muted-foreground">
                        <GraduationCap className="w-3.5 h-3.5" />{profile.student_id}
                      </div>
                    )}
                    {profile.major && (
                      <div className="flex items-center gap-1.5 text-xs bg-muted/80 rounded-full px-3 py-1.5 text-muted-foreground">
